...
from fuel_mirror.common.url_parser import get_url_parser
...
class BaseCommand(command.Command):
    ...
    def get_repositories(self, parsed_args):
    ...
    def get_repositories_for_fuel(self, parsed_args):
        ...
        result = dict()
        for config in self.get_repositories(parsed_args):
            name = config["name"]
...
class CopyRepositoryCommand(BaseCommand):
    ...
    def copy_repositories(self, parsed_args):
        ...
        total = None
        for repo_config in self.get_repositories(parsed_args):
...
class FuelCommandMixin(object):
    ...
    def update_clusters(self, repositories, ids=None):
        ...
        for cluster in clusters:
            ...
            if modified:
                self.app.LOG.debug("Try to update cluster attributes: %s", modified)
    ...
    def update_repository_settings(self, attributes, repositories):
