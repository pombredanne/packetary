...
class YumRepositoryDriver(RepositoryDriverBase):
    ...
    def _load_db(self, connection, baseurl, repomd, kind):
        """Loads database by type."""
        self.logger.debug("loading %s database...", kind)
        node = repomd.find(
        ...
        self.logger.debug("loading %s - %s...", kind, url)
        return etree.parse(
