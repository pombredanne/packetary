...
def find_package(repo, name, search):
    ...
    versions = repo.find_package(name)
...
def get_external_depends(index, unresolved):
    ...
    for package in repo:
        for d in package.get_depends():
            ...
            while alt is not None:
                if find_package(repo, alt.package, alt.condition):
                    break
                alt = alt.alt
...
def filter_by_depends(repo, depends):
    ...
    for dep in queue_iterator(depends):
        package = find_package(repo, dep.package, dep.condition)
